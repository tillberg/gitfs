...
function withLock(pathToLock, action, cb) {
...
function cloneGen (self) {
  return function (cb) {
    self.git.ready(function() {
      withLock(self.basePath, function(done) {
...
Repo.gitExec = function (args, cb) {
  ...
  this.clone(function() {
    ...
    exec('git', args, { cwd: self.basePath, quietError: true }, errorHandler('Error on git' + args[0] + ': ERR, ARG0 ARG1', cb, function(err, stdout, stderr) {
